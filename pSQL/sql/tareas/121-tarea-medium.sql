...
select count(a.created_at), b.user_id, b.name
from posts a
inner join users b on a.created_by=b.user_id
group by b.user_id
order by count(a .created_at) desc
limit 10;
...
select count(a.*)
from users a
left outer join user_lists b on a.user_id=b.user_id
where b.user_list_id is null;
...
select json_agg(json_build_object(user_id, content)) 
from comments
where comment_parent_id=1;
...
select a.*,
	(select json_agg(json_build_object(b.user_id, b.content))
	 from comments b
