...
---funcion para calcular el maximo a aumentar de salario
create or replace function max_raise(codigo int)
returns numeric(8,2)
as
$$
declare 
...
----multiples queries en la funcion
CREATE
OR REPLACE FUNCTION MAX_RAISE2 (CODIGO INT) RETURNS NUMERIC(8, 2) AS $$
declare
	employee_job_id int;
...
begin
	...
	if (possible_raise<0) then
		raise exception 'Persona con salario mayor al máximo de; % ', codigo;
		--possible_raise=0;
	end if;
...
CREATE
OR REPLACE PROCEDURE INSERT_REGION (COD INT, NOMBRE VARCHAR) AS $$
begin
...
$$ LANGUAGE PLPGSQL;
call insert_region(5,'Mamaguevo');